...
# install hugo
RUN curl -LO https://github.com/gohugoio/hugo/releases/download/v0.158.0/hugo_extended_0.158.0_linux-amd64.tar.gz --output /tmp/hugo.tgz && \
  cd /tmp && tar xzf hugo.tgz && mv hugo /usr/local/bin/hugo && chmod a+x /usr/local/bin/hugo && rm -f hugo.tgz && cd -
