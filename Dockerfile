...
RUN apk update && apk upgrade --no-cache && apk add --no-cache libc6-compat g++ curl ca-certificates nodejs npm git make tar docker-cli bash jq grep zstd pandoc-cli mkdocs tectonic font-urw-base35 kubectl openssl softhsm openssh-client-default
...
# add node apps
RUN npm install -g renovate @quasar/cli wrangler @usebruno/cli linkinator
