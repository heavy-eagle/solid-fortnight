...
RUN apk update && apk upgrade --no-cache && apk add --no-cache libc6-compat g++ curl ca-certificates node npm git make tar docker-cli bash jq grep zstd pandoc-cli mkdocs tectonic font-urw-base35 kubectl openssl
